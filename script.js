...
const dayHtml = `<div class="coach-card p-6 animate-card ${colorClass}" id="${dayId}"><button onclick="removeElement('${dayId}')" class="absolute top-5 left-5 text-slate-600 hover:text-red-500 transition-colors"><i class="fas fa-trash-alt text-sm"></i></button><div class="flex items-center gap-3 mb-6"><div class="w-8 h-8 rounded-lg dynamic-bg flex items-center justify-center"><i class="fas fa-dumbbell dynamic-text text-sm"></i></div><input type="text" oninput="saveDraft()" value="${title}" placeholder="عنوان القسم..." class="section-title-input bg-transparent font-black dynamic-text w-full border-none outline-none text-lg p-0 placeholder-slate-700 focus:ring-0"></div><div id="ex-container-${dayCount}" class="space-y-4"></div><button onclick="addExercise(${dayCount})" class="mt-6 w-full py-3 bg-slate-800/40 text-slate-400 rounded-xl text-[11px] font-black border border-white/5 hover:dynamic-bg hover:dynamic-text transition-all uppercase">+ إضافة تمرين جديد</button></div>`;
...
const exHtml = `<div class="exercise-item p-4 rounded-xl space-y-3 transition-all" id="${exId}"><div class="flex justify-between items-center"><div class="flex items-center gap-2"><i class="fas fa-bolt dynamic-text text-[10px]"></i><span class="text-[9px] font-black text-slate-500 uppercase tracking-widest">تمرين ${currentCount}</span></div><button onclick="document.getElementById('${exId}').remove(); saveDraft();" class="text-slate-700 hover:text-red-400 transition-colors"><i class="fas fa-times text-xs"></i></button></div><input type="text" oninput="saveDraft()" value="${name}" placeholder="اسم التمرين..." class="ex-name-input w-full bg-transparent text-sm font-bold border-none outline-none p-0 text-slate-200 placeholder-slate-700 focus:ring-0"><div class="grid grid-cols-2 gap-3 pt-1"><div class="space-y-1"><span class="text-[8px] font-black text-slate-500 uppercase flex items-center gap-1"><i class="fas fa-redo dynamic-text"></i> جولات</span><input type="number" oninput="saveDraft()" value="${rounds}" placeholder="0" class="w-full rounded-lg p-2 text-center text-sm font-black outline-none focus:border-slate-500 transition-all bg-slate-900 border border-white/5"></div><div class="space-y-1"><span class="text-[8px] font-black text-slate-500 uppercase flex items-center gap-1"><i class="fas fa-list-ol dynamic-text"></i> تكرار</span><input type="number" oninput="saveDraft()" value="${reps}" placeholder="0" class="w-full rounded-lg p-2 text-center text-sm font-black outline-none focus:border-slate-500 transition-all bg-slate-900 border border-white/5"></div></div></div>`;
